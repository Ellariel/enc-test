...
class LNEnv(Env): 
    def __init__(self, G, transactions, observation_size=64, train=True) -> None:
        ...
        self.enc = Enc(n = self.n,
                       latent_dim = self.observation_size)
        self.enc.load()
        
        self.observation_space = spaces.Box(low=-1, high=1, shape=(self.observation_size, ), dtype=np.float32)
        self.action_space = spaces.Box(low=-1, high=1, shape=(self.n + 1, ), dtype=np.float32)
    ...
    def reset(self):
        ...
        self.current_observation = np.zeros((self.n, ), dtype=np.float32)
